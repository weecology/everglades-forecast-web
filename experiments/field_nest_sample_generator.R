...
# Load field nests
field_nests <- read.csv("https://raw.githubusercontent.com/weecology/EvergladesWadingBird/main/Nesting/UAV_Flagged_Nest_Coordinates.csv")

# Load field nest samples
sample_locations <- read.csv("experiments/field_nest_sample_locations.csv")
...
if (changes_made){
  ...
  write.csv(samp_locs_updated, "experiments/field_nest_sample_locations.csv", row.names = FALSE)
} else {
